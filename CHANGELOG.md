...
- **Retry** (#26). Failing scenarios can be retried in the same run: `config :cucumber, retry: n` sets a global limit, and a `@retry-n` tag (scenario- or feature-level) overrides it — including `@retry-0` to exempt a scenario. Each attempt re-runs the full lifecycle (before hooks, background, steps, after hooks) with a fresh context carrying the 1-based `:retry_attempt`; the scenario passes if any attempt passes, and every retry prints a one-line flake warning. Undefined, ambiguous, and pending scenarios never retry (CCK semantics — they cannot succeed by repetition).
